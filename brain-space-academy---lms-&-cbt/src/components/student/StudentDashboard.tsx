...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  // Exam Performance Chart Filter (Category & Chart Type)
  ...
  // Slide Carousel Auto Advance Effect (Vertical Slide 8 Detik)
  useEffect(() => {
  ...
  // Filter student latest results based on chartCategoryFilter, sorted so most updated exam is on the right
  const filteredChartResults = latestStudentResults
    .filter(r => chartCategoryFilter === 'ALL' || r.examCategory === chartCategoryFilter)
  ...
  // All results (all attempts) for the selected category filter
  const allCategoryStudentResults = studentResults.filter(
    r => chartCategoryFilter === 'ALL' || r.examCategory === chartCategoryFilter
  );

  // Metrics for selected category: Skor Terbaik diambil dari SEMUA data ujian dan percobaan
  const bestScoreResult = allCategoryStudentResults.length > 0
    ? allCategoryStudentResults.reduce((max, r) => (r.score > max.score ? r : max), allCategoryStudentResults[0])
    : null;

  const bestScoreValue = bestScoreResult ? bestScoreResult.score : 0;
  const bestScoreExamTitle = bestScoreResult ? bestScoreResult.examTitle : '-';
  const bestScoreAttempt = bestScoreResult ? attemptInfoMap.get(bestScoreResult.id) : null;
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-5 shadow-2xl flex flex-col justify-between relative overflow-hidden space-y-3">
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-2.5 gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">Slide Vertikal Otomatis (8 Detik)</p>
                  </div>
                </div>

                {/* Slide Controls Vertikal */}
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    onClick={() => setIsAutoPlay(!isAutoPlay)}
                    title={isAutoPlay ? 'Jeda Slide Otomatis (8d)' : 'Putar Slide Otomatis (8d)'}
                  ...
                  <button
                    ...
                    title="Slide Ke Atas (Sebelumnya)"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl border border-slate-700 transition-all"
                    title="Slide Ke Bawah (Selanjutnya)"
                  >
              ...
              {/* Tampilan Program Unggulan dengan Flow Vertikal Perlahan Mengalir (Top-to-Bottom) */}
              {activeProgramsList.length > 0 && (() => {
                const firstIdx = currentSlide;
                const secondIdx = (currentSlide + 1) % activeProgramsList.length;
                const thirdIdx = (currentSlide + 2) % activeProgramsList.length;
                const visiblePrograms = [
                  activeProgramsList[firstIdx],
                  activeProgramsList.length > 1 ? activeProgramsList[secondIdx] : null,
                  activeProgramsList.length > 2 ? activeProgramsList[thirdIdx] : null
                ].filter(Boolean) as FeaturedProgram[];

                // Infinite loop list for continuous smooth flow animation
                const flowList = [...activeProgramsList, ...activeProgramsList, ...activeProgramsList];

                return (
                  <div className="space-y-3 flex-1 flex flex-col justify-between overflow-hidden relative">
                    {/* Top & Bottom Subtle Fade Gradient Masks for Smooth Flowing Viewport */}
                    <div className="absolute top-0 left-0 right-0 h-3 bg-gradient-to-b from-slate-900 to-transparent z-10 pointer-events-none" />
                    <div className="absolute bottom-0 left-0 right-0 h-3 bg-gradient-to-t from-slate-900 to-transparent z-10 pointer-events-none" />

                    {/* Flowing Container */}
                    {isAutoPlay ? (
                      <div className="max-h-[480px] overflow-hidden relative">
                        <div
                          className="space-y-3 animate-vertical-flow"
                          style={{ animationPlayState: isAutoPlay ? 'running' : 'paused' }}
                        >
                          {flowList.map((prog, idx) => (
                            <div
                              key={`flow-${prog.id}-${idx}`}
                              className="bg-slate-950/85 border border-slate-800/90 rounded-2xl p-3 sm:p-3.5 flex flex-row gap-3 items-center hover:border-cyan-500/50 transition-all shadow-md group"
                            >
                              {/* SEBELAH KIRI: Banner Foto di atas, Informasi & Deskripsi di bawah foto */}
                              <div className="flex-1 min-w-0 flex flex-col justify-between">
                                {/* Banner Foto */}
                                <div className="relative w-full h-24 sm:h-28 rounded-xl overflow-hidden bg-slate-900 border border-slate-800 shrink-0">
                                  <img
                                    src={prog.thumbnail}
                                    alt={prog.title}
                                  ...
                                  <span className="absolute top-1.5 left-1.5 px-2 py-0.5 rounded-md text-[9px] font-black bg-gradient-to-r from-red-600 to-blue-600 text-white shadow-sm">
                                    {prog.badge || 'VIP'}
                                  </span>
                                  <span className="absolute bottom-1.5 left-1.5 right-1.5 text-[9px] sm:text-[10px] font-bold text-cyan-300 truncate">
                                    {prog.category}
                                  </span>
                                </div>

                                {/* Informasi & Deskripsi di Bawah Foto */}
                                <div className="mt-2 space-y-0.5">
                                  <h4 className="font-extrabold text-white text-xs sm:text-sm line-clamp-1 group-hover:text-cyan-300 transition-colors">
                                    {prog.title}
                                  </h4>
                              ...
                              {/* SEBELAH KANAN: Tombol Aksi Stacked (Atas: Daftar, Bawah: Lihat Selengkapnya) */}
                              <div className="flex flex-col gap-2 shrink-0 justify-center w-28 sm:w-32">
                                <a
                                  href={prog.registerUrl}
                                  target="_blank"
                                  rel="noreferrer"
                                  className="w-full px-2.5 py-2 bg-gradient-to-r from-red-600 via-blue-600 to-blue-700 hover:from-red-500 hover:to-blue-600 text-white font-extrabold text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-1 text-center"
                                >
                                  <span>Daftar</span>
                                ...
                                <button
                                  ...
                                  className="w-full px-2.5 py-2 bg-slate-900 hover:bg-slate-800 text-slate-200 font-bold text-[11px] rounded-xl border border-slate-700 transition-all flex items-center justify-center gap-1 text-center"
                                >
                      ...
                      <div key={`slide-vert-${currentSlide}`} className="space-y-3 animate-slide-down">
                        {visiblePrograms.map((prog, idx) => (
                          <div
                            key={`${prog.id}-${idx}`}
                            className="bg-slate-950/80 border border-slate-800/90 rounded-2xl p-3 sm:p-3.5 flex flex-row gap-3 items-center hover:border-slate-700 transition-all shadow-md group"
                          >
                            {/* SEBELAH KIRI: Banner Foto di atas, Informasi & Deskripsi di bawah foto */}
                            <div className="flex-1 min-w-0 flex flex-col justify-between">
                              {/* Banner Foto */}
            ...
            {/* COLUMN 2 (RIGHT): GRAFIK ANALISIS PERFORMA UJIAN TERUPDATE */}
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-5 shadow-2xl flex flex-col justify-between space-y-3">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">Hasil Terupdate Per Ujian Dalam Kategori</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
                  ...
                  <select
                    ...
                    className="bg-slate-950 border border-slate-800 text-xs text-cyan-300 font-bold rounded-xl px-2.5 py-1.5 focus:border-cyan-500 outline-none w-full sm:w-auto max-w-[180px] truncate"
                  >
              ...
              <div className="grid grid-cols-3 gap-2 text-center bg-slate-950/80 p-2.5 rounded-2xl border border-slate-800">
                <div className="p-1 flex flex-col justify-between">
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider flex items-center justify-center gap-1">
                      <Award className="w-3.5 h-3.5 text-amber-400 shrink-0" /> Skor Terbaik
                    </p>
                    <p className="text-base sm:text-lg font-black text-amber-400 mt-0.5">{bestScoreValue} Poin</p>
                  </div>
                  <p className="text-[9px] sm:text-[10px] text-cyan-300 font-semibold leading-tight line-clamp-2 mt-1 w-full text-center px-0.5 break-words" title={bestScoreExamTitle}>
                    {bestScoreResult
                      ? `🏆 ${bestScoreExamTitle}${bestScoreAttempt && bestScoreAttempt.totalAttempts > 1 ? ` (Tes #${bestScoreAttempt.attemptNumber})` : ''}`
                      : 'Belum Ada Ujian'}
                  </p>
                </div>

                <div className="p-1 border-x border-slate-800 flex flex-col justify-between">
                  <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-medium">Hasil Terupdate</p>
                </div>
                ...
                <div className="p-1 flex flex-col justify-between">
                  ...
                  <p className="text-[10px] text-slate-500 font-medium">Sesuai Kriteria</p>
                </div>
              </div>

              {/* Pilihan Tipe Grafik (Diagram Batang vs Diagram Garis) Langsung di Atas Grafik */}
              <div className="flex items-center justify-end bg-slate-950/90 px-3 py-1.5 rounded-2xl border border-slate-800">
                <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800">
                  <button
                    type="button"
                    onClick={() => setChartType('BAR')}
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-extrabold transition-all flex items-center gap-1.5 ${
                      chartType === 'BAR'
              ...
              {/* Diagram Visual (Batang vs Garis) */}
              {barChartData.length > 0 ? (
                <div className="h-56 sm:h-64 w-full bg-slate-950/60 p-2.5 rounded-2xl border border-slate-800/80 pt-3">
                  <ResponsiveContainer width="100%" height="100%">
                    {chartType === 'LINE' ? (
                      <LineChart data={barChartData} margin={{ top: 15, right: 15, left: -20, bottom: 20 }}>
                        ...
                        <Line
                          type="monotone"
                          dataKey="score"
                          name="Skor Akhir Ujian"
                          stroke="#38bdf8"
                          strokeWidth={3}
                          dot={{ fill: '#0284c7', r: 5, stroke: '#38bdf8', strokeWidth: 2 }}
                ...
                <div className="h-56 sm:h-64 w-full bg-slate-950/60 rounded-2xl border border-slate-800 flex flex-col items-center justify-center p-6 text-center space-y-3">
                  ...
                  <div className="space-y-1 max-w-xs">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed">
                      Selesaikan ujian pada kategori <span className="text-cyan-400 font-semibold">{chartCategoryFilter === 'ALL' ? 'apapun' : chartCategoryFilter}</span> untuk menampilkan grafik diagram {chartType === 'LINE' ? 'garis skor akhir' : 'batang skor'} terupdate.
                    </p>
                  </div>
                </div>
              )}
