...
import { Exam, ClassItem, ExamCategory, ExamMode, Question, QuestionType } from '../../types';
import {
  FileCheck2,
  Plus,
  Key,
...
export const ExamManagement: React.FC<ExamManagementProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const savedExam: Exam = {
      ...
      mode,
      pdfDriveUrl: mode === 'EMBED_DRIVE_PDF' ? pdfDriveUrl.trim() : undefined,
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto pr-2 space-y-6 custom-scrollbar">
              ...
              <div className="bg-slate-950/80 border border-slate-800/80 p-4 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {mode === 'EMBED_DRIVE_PDF' && (
                    <div>
                      <label className="block text-xs font-semibold text-slate-300 mb-1">Link Embed PDF Google Drive</label>
                      <input
                        type="url"
                        value={pdfDriveUrl}
                        onChange={e => setPdfDriveUrl(e.target.value)}
                        placeholder="https://drive.google.com/file/d/1Bzx7tT3.../preview"
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                          <div className="sm:col-span-8 space-y-1.5">
                            <div className="flex flex-wrap items-center justify-between gap-1">
                              <label className="block text-[11px] font-semibold text-slate-300">Naskah / Teks Soal</label>
                              
                              {/* Alignment Selector */}
                              <div className="flex items-center gap-1 bg-slate-950 p-0.5 rounded-lg border border-slate-800">
                                <span className="text-[10px] text-slate-500 font-medium px-1">Rata:</span>
                                <button
                                  type="button"
                                  onClick={() => updateQuestion(idx, { textAlign: 'left' })}
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${(!q.textAlign || q.textAlign === 'left') ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                                  title="Rata Kiri"
                                >
                                  Kiri
                                </button>
                                <button
                                  type="button"
                                  onClick={() => updateQuestion(idx, { textAlign: 'center' })}
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${(q.textAlign === 'center') ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                                  title="Rata Tengah"
                                >
                                  Tengah
                                </button>
                                <button
                                  type="button"
                                  onClick={() => updateQuestion(idx, { textAlign: 'right' })}
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${(q.textAlign === 'right') ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                                  title="Rata Kanan"
                                >
                                  Kanan
                                </button>
                                <button
                                  type="button"
                                  onClick={() => updateQuestion(idx, { textAlign: 'justify' })}
                                  className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${(q.textAlign === 'justify') ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                            ...
                            {/* Math Symbol Toolbar */}
                            <div className="flex items-center gap-1 overflow-x-auto py-1 px-2 bg-slate-950 rounded-lg border border-slate-800 custom-scrollbar">
                              <span className="text-[10px] font-bold text-cyan-400 shrink-0 mr-1">Simbol MTK:</span>
                              {['√', 'x²', 'xⁿ', 'x₁', '½', '±', 'π', 'θ', 'Σ', '∫', 'α', 'β', 'Δ', '∞', '≤', '≥', '≠', '÷', '×', '∈', '°', 'log', 'lim'].map(sym => (
                                <button
                                  key={sym}
                                  type="button"
                                  onClick={() => updateQuestion(idx, { text: (q.text || '') + sym })}
                                  className="px-1.5 py-0.5 bg-slate-900 hover:bg-cyan-900/60 border border-slate-800 hover:border-cyan-700/60 text-slate-200 hover:text-cyan-200 rounded font-mono text-[11px] shrink-0 transition-colors"
                                  title={`Sisipkan ${sym}`}
                                >
                                  {sym}
                                </button>
                            ...
                            <textarea
                              ...
                              className={`w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-white placeholder-slate-600 focus:border-cyan-500 ${
                                q.textAlign === 'center' ? 'text-center' :
                                q.textAlign === 'right' ? 'text-right' :
                                q.textAlign === 'justify' ? 'text-justify' : 'text-left'
                              }`}
                            />
                          </div>
                          ...
                          <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800/80 space-y-2">
                            ...
                            <div className="space-y-2">
                              {['A', 'B', 'C', 'D', 'E'].map(optKey => {
                                ...
                                return (
                                  <div key={optKey} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-slate-900/90 p-2 rounded-xl border border-slate-800">
                                    <span className="w-6 h-6 rounded-md bg-slate-800 font-bold text-cyan-400 text-xs flex items-center justify-center shrink-0">
                                      {optKey}
                                    </span>
                                    ...
                                    <input
                                      ...
                                      onChange={e => updateQuestionOption(idx, optKey, 'text', e.target.value)}
                                      placeholder={`Teks Opsi ${optKey}...`}
                                      className="flex-1 bg-slate-950 border border-slate-800 rounded-lg p-1.5 text-xs text-white placeholder-slate-600"
                                    />
                                    ...
                                    <input
                                      ...
                                      className="w-full sm:w-56 bg-slate-950 border border-slate-800 rounded-lg p-1.5 text-[10px] text-slate-300 placeholder-slate-600"
                                    />

                                    {currentOpt.imageUrl && (
                                      <img src={currentOpt.imageUrl} alt={`Preview Opsi ${optKey}`} className="w-8 h-8 rounded border border-slate-700 object-cover shrink-0" />
                                    )}
                                  </div>
                                );
                              })}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 pt-1 border-t border-slate-800/60 items-center">
                          ...
                          <div className="sm:col-span-4">
                            <input
                              ...
                              placeholder="Pembahasan Soal Singkat..."
                              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-1.5 text-[11px] text-slate-300 placeholder-slate-600"
                            />
                          </div>
                        </div>
                      </div>
              ...
              <div className="pt-2 flex gap-3">
                <button
                  ...
                  className="w-1/3 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold rounded-2xl text-xs"
                >
                  Batal
                </button>
                ...
                <button
                  type="submit"
                  className="w-2/3 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold rounded-2xl text-xs shadow-lg shadow-cyan-600/30 flex items-center justify-center gap-2"
                >
